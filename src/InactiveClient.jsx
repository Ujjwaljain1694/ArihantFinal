...
export default function InactiveClient() {
  ...
  const fetchInactiveClients = async (query) => {
    try {
      setLoading(true);
      setError(null);
      // Fetch inactive clients from UAT API with clientCode search filter
      const res = await korpInstance.get("/dashboard/korpgetclientDetail", {
        params: { pageNumber: 0, size: 500, Type: "IC", clientCode: query || undefined }
      });
